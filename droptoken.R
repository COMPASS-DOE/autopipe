...
# We need a long-lived Dropbox token
# This solution is from
# https://stackoverflow.com/questions/71393752/get-a-refresh-token-for-dropbox-api-using-rdrop2-and-drop-auth

# "Currently, rdrop2's drop_auth() function will generate a token with these
# credentials: <credentials> access_token, token_type, expires_in, uid,
# account_id. Since access tokens last only 4 hours now, we will need a refresh
# token as a part of our credentials so that we may get a new access token
...
saveRDS(refreshable_token, "refreshable_droptoken.rds")
